...
Docker Desktop currently supports Windows 10 64-bit version 22H2 (build 19045) for supported Enterprise, Pro, or Education editions, and Windows 11 64-bit version 23H2 (build 22631) or later for those editions. Docker also requires WSL 2 version 2.1.5 or later when using the WSL 2 backend, along with a 64-bit processor supporting SLAT, at least 8 GB RAM, and hardware virtualization enabled. :contentReference[oaicite:1]{index=1}

Docker recommends the **per-user installation** for most Windows users. This installation mode does not normally require administrator privileges to install or update Docker Desktop. Some system-level operations can still require administrator access. :contentReference[oaicite:2]{index=2}

> **Important**
>
> Docker Desktop licensing depends on how it is used.
>
> Docker states that Docker Desktop is free for personal use, education, non-commercial open source projects, and small businesses that meet its stated requirements. Larger organisations and some commercial or government use cases require a paid subscription.
>
> If you are installing Docker on a company computer, check your organisation's software licensing and security policies before you continue. :contentReference[oaicite:3]{index=3}
...
Compare the information with the current Docker Desktop system requirements before continuing. :contentReference[oaicite:4]{index=4}
...
Docker recommends at least **8 GB of RAM** for the current Windows WSL 2 requirements. :contentReference[oaicite:5]{index=5}
...
Docker Desktop requires hardware virtualization for the Windows configurations described by Docker. :contentReference[oaicite:6]{index=6}
...
WSL 2 provides a Linux environment that Docker Desktop can use to run Linux containers. Docker currently requires WSL 2 version 2.1.5 or later for this backend. :contentReference[oaicite:7]{index=7}
...
Docker's current documentation recommends keeping WSL up to date and requires at least version 2.1.5 for Docker Desktop's WSL 2 backend. :contentReference[oaicite:8]{index=8}
...
Docker currently provides Windows x86_64 and ARM options. :contentReference[oaicite:9]{index=9}
...
Docker's current Windows installer supports a per-user installation mode, which is recommended for most users. The installer can also be configured for an all-users installation when required. :contentReference[oaicite:10]{index=10}
...
Docker Desktop will not run until the agreement is accepted. :contentReference[oaicite:11]{index=11}
...
Docker currently requires WSL 2 version 2.1.5 or later for the WSL 2 backend. :contentReference[oaicite:12]{index=12}
